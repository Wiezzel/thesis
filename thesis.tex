...
\subsubsection*{Weka}
The \textsc{Waikato Environment for Knowledge Analysis} (WEKA) \cite{weka} is an open source data mining software suite written in Java. It contains a broad collection of state-of-the-art machine learning algorithms and can be used either as a stand-alone application, or as a library. \textsc{ChangeAnalyzer} utilizes \textsc{Weka} to train various predictive models (described in Section \ref{sec:machine_learning}) and predict the defect-proneness of unclassified methods.

\subsection*{LIBSVM}

LIBSVM \cite{libsvm} is a library for \emph{support vector machines} (see Section \ref{sec:svm}) in C++ and Java. It supports classification (C-SVC, $\nu$-SVC), regression ($\varepsilon$-SVR, $\nu$-SVR) and distribution estimation (one-class SVM). LIBSVM bindings are provided for many languages and frameworks (\textit{e.g.} Python, R, Ruby). \textsc{ChangeAnalyzer} uses a wrapper interface compatible with \textsc{Weka}~\cite{weka_libsvm}.

\subsection{R}

R \cite{r} is a free, cross-platform software environment for statistical computations. It is not used for the implementation of \textsc{ChangeAnalyzer} itself, but of auxiliary scripts (see Section \ref{sec:set-up}) developed for the evaluation of the algorithms presented in Section \ref{sec:machine_learning}.
...
\subsubsection*{e1071}
\textsc{e1071} \cite{e1071} is an R package containing an implementation of \emph{support vector machine} algorithm described in Section \ref{sec:svm}.
...
\textsc{ChangeAnalyzer} includes a command-line interface for users. Section \ref{sec:main_interface} presents the main functionalities offered by \textsc{ChangeAnalyzer} as an integrated tool. Section \ref{sec:test_interface} describes a test interface, which provides more limited capabilities and was employed during system development and testing.

\subsection{Main interface}
\label{sec:main_interface}

The main interface is provided by the main class of the \textsc{ChangeAnalyzer} runnable JAR. In order to use it, one must simply run the following command:
\begin{verbatim}
